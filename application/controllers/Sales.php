...
class Sales extends Secure_Controller
{
	...
	public function complete()
	{
		...
		$this->session->set_userdata('exchange_rate_set', $exchange_rate_set);
		...
		if($this->sale_lib->is_invoice_mode() || $pos_invoice)
		{
			...
			else
			{
				...
				$data['sale_id_num'] = $this->Sale->save($sale_id, $data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'],
					...
					$number_locale_alt, $currency_symbol_alt);
	...
	public function receipt_alt($sale_id)
	{
	...
	public function invoice_alt($sale_id)
	{
		$data = $this->_load_sale_data($sale_id, TRUE);
		$this->load->view('sales/'.$data['invoice_view'], $data);
		$this->sale_lib->clear_all();
